    ...
    extends ElementTransformCopyBase
{
    ...
    protected Iterator<Var> varGen;

    public ElementTransformDatasetDescription(Stack<Node> graphs, Iterator<Var> varGen, ExprList defaultGraphExprs, ExprList namedGraphExprs) {
        this.graphs = graphs;
        this.varGen = varGen;
    ...
    public static ElementTransformDatasetDescription create(Stack<Node> graphs, Element e, DatasetDescription dd, Map<String, Expr> graphToExpr) {
        ...
        Iterator<Var> varGen = VarGen.of("__dg__").filter(v -> !vars.contains(v)).iterator();

        // VarGeneratorBlacklist.create("__dg_", vars);
...
//        namedGraphExprs.add(NodeValue.makeNode(NodeFactory.createURI("ng2")));
    ...
    public static Element applyGraphs(Iterator<Var> varGen, Node gn, Element elt1, ExprList exprs) {
